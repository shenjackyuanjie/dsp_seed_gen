...
impl AstroOrbitData {
    ...
    pub fn predict_pose(&mut self, time: i64, center: VectorLF3, astro_data: &mut AstroData) {
        ...
        num2 *= 2.0 * PI;
        astro_data.u_pos = self.orbit_rotation.rotation_lf(&VectorLF3::new(num2.cos(), 0.0, num2.sin()));
        astro_data.u_rot = Quaternion::look_rotation(&self.orbit_normal, &astro_data.u_pos);
        astro_data.u_pos = astro_data.u_pos * num + center;
        let mut num4 = (time as f64 + 1.0) / (self.orbital_period * 60.0) + self.orbit_phase as f64 / 360.0;
        let num5 = (num4 + 0.1).floor();
        num4 -= num5;
        // *= 6.283185307179586
        num4 *= 2.0 * PI;
        astro_data.u_pos_next = self.orbit_rotation.rotation_lf(&VectorLF3::new(num4.cos(), 0.0, num4.sin()));
