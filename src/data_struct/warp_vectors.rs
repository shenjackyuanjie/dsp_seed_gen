...
use nalgebra::{Vector2, Vector3, Vector4, UnitQuaternion, Unit};
...
macro_rules! short_impl {
    ($vec_type: ty, $f_type: ty) => {
        ...
        pub fn move_towards(current: Self, target: Self, max_distance_delta: $f_type) -> Self {
            ...
            Self::zeros() + delta / magnitude * max_distance_delta
        }
...
impl Quaternion {
    ...
    pub fn look_rotation(forward: VectorLF3, up: VectorLF3) -> Quaternion {
        ...
        let mut quaternion = Quaternion::zeros();
    ...
    pub fn angle_axis(angle: f32, axis: VectorF3) -> Quaternion {
        let angle = angle.to_radians();
        let unit = Unit::new_normalize(axis.vec);
        let mut qua = UnitQuaternion::from_axis_angle(&unit, angle);
        let mut vec = qua.as_vector();
    }
}
...
#[test]
fn zeros() {
    let v1 = VectorLF2::zeros();
    let v2 = VectorLF3::zeros();
    let v3 = VectorLF4::zeros();
    let v4 = VectorF2::zeros();
    let v5 = VectorF3::zeros();
    let v6 = VectorF4::zeros();
